...
public class PostgreSqlQuery implements IDbQuery {
    ...
    public String tablesSql(DataSourceConfig dataSourceConfig) {
        String tablesql = "SELECT A.tablename, obj_description(relfilenode, 'pg_class') AS comments FROM pg_tables A, pg_class B WHERE A.schemaname='%s' AND A.tablename = B.relname";
        ...
        return String.format(tablesql, schema);
    }
